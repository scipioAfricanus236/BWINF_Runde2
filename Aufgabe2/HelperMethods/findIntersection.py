...
class FindIntersection:
    def __init__(self, lines, margin=0):
        #because the lines / obstacles do not change we can store it like this
        self.lines = lines
        self.margin = margin

    def isThereAnyIntersection(self):
        for line in self.lines:
            if self.isItPossible(line) == True:
                return True
        return False
    ...
    def isItPossible(self, road):
        ...
        for line in self.lines:
            if (line != road and line[0] != road[1] and line[0] != road[0]):  
                if self.findIntersection(line, road, self.margin):
                    return True
            
        return False

    def findIntersection(self, a, b, margin=0):
        ...
        if len(functionA) == 2:
            if len(functionB) == 2:
                ...
                if (x - margin) > a[0][0] and (x + margin) < a[1][0] and (x - margin) > b[0][0] and (x + margin) < b[1][0]:
                    return True
                else:
                    return False
            else:
